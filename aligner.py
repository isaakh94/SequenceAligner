...
def needle_affine(A, B, matrix = PAM250, gap = -1, opengap = -5, EMBOSS = False, global_align = True, local_align = False):
    ...
    for i in range(1, len(A)+1):
        for j in range(1, len(B)+1):
            options = [scores_matched[i-1][j-1][0],
                       scores_hgap[i-1][j-1][0],
                       scores_vgap[i-1][j-1][0],
                       zero_option]
            scores_matched[i][j] = [matrix[A[i-1]][B[j-1]] + max(options),
                                    directions[np.argmax(options)]]
            options = [scores_matched[i-1][j][0] + opengap + gap*maybe_gap,
                       scores_hgap[i-1][j][0] + gap,
                       -np.inf]
            scores_hgap[i][j] = [max(options), directions[np.argmax(options)]]
            options = [scores_matched[i][j-1][0] + opengap + gap*maybe_gap,
                       -np.inf,
                       scores_vgap[i][j-1][0] + gap]
...
def main():
    ...
    parser.add_argument('-c', '--charbreak', type = int, default = 60, help = 'How many characters per row you want the output to have.')
